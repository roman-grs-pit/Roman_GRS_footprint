"""
Given some input file (assumed to be fits) and input tiling file
(in ecsv format), determine the number of times each ra,dec in the input
will be observed by the grism with the assumed wavelength coverage
requires that the optical model be installed and the environment variable
ROMAN_GDPS_OPTICAL_MODEL_CONFIG be set to the path of the yaml config file
for the optical model
Also requires astropy to be installed

***
Example install and run on SMDC east:
Followed instructions here for miniforge3 to allow conda: https://github.com/RomanSpaceTelescope/RST_SMDC_users/blob/main/documentation/miniforge.md
Needed to add to end of ~/.bashrc file to allow local installs to be found
export PATH="$HOME/.local/bin:$PATH"

conda create --name test_footprint python
conda activate test_footprint
python -m pip install astropy

***github directory at /data/ajross/github/, change below to wherever you want to clone the repos
export github_dir=/data/ajross/github

cd $github_dir
git clone https://github.com/RomanSpaceTelescope/GDPS_optical_model.git
cd GDPS_optical_model/
python -m pip install -e .

cd $github_dir
git clone https://github.com/roman-grs-pit/Roman_GRS_footprint.git
cd Roman_GRS_footprint/
python -m pip install -e .
...
python $github_dir/Roman_GRS_footprint/scripts/get_intile_coverage_mkran.py

to run the full survey run with --fullsurvey
"""
...
parser.add_argument("--nran", help="number of randoms to use",
...
parser.add_argument(
    "--target",
    help=(
        "which targets to include based on TARGET_NAME in tile file "
        "(ignored if not in tile file), options are medium (default, "
        "includes all except XMM and COSMOS), xmm, cosmos"
    ),
    default="medium",
...
parser.add_argument("--palist", help="list of PA values for repeated RA,DEC",
...
if args.tilefile is None:
    ...
    args.tilefile = os.path.join(
        github_dir, "Roman_GRS_footprint", "data", "994-hlwas-Feb26_fixed_orients.sim.ecsv")
...
if args.fullsurvey:
    ...
    rax = args.ramax
    logger.info(
        "will apply ra,dec bounds of ra > "
        + str(ram)
        + " and ra < "
        + str(rax)
        + " and dec > "
...
ran4degfn = args.outroot + "/DESIran" + \
    str(args.nran) + str(ram) + str(rax) + str(decm) + str(decx) + ".fits"
...
if args.fullsurvey:
    outdir = args.outroot + "/test" + "/fullsurvey" + wfistr + "/"
else:
    outdir = args.outroot + "/test" + "/ramin" + \
        str(ram) + "decmin" + str(decm) + wfistr + "/"
...
for chunk in range(0, Nchunk):
    ...
    def get_idx_tl(tl):
        ...
        if len(ran_indices_tl) > 0:
            ...
            for det in dets:
                xpixl, ypixl = trace_on_det.optmod.coords.convert_fpa_to_sca(
                    xfpa, yfpa, sca=det)
                selp = xpixl > -1000
                selp &= xpixl < 5088
                selp &= ypixl > -1000
    ...
    # This does not work as a bitmask because some points are observed by the
    # same detector in multiple tiles. Reducing to a mapping of detectors that
    # see each point would require tracking repeated hits per detector, which
    # is more complicated and may not be worth it for this application.
