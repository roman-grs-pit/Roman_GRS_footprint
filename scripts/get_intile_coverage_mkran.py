...
parser.add_argument("--outname", help="additional name for output files", default="test")
...
ran4degfn = args.outroot + "/DESIran" + str(args.nran) + str(ram) + str(rax) + str(decm) + str(decx) + ".fits"

data = Table()
ra, dec = sky_coords.generate_randoms(nran=args.nran, ra_bounds=(ram, rax), dec_bounds=(decm, decx))
data["RA"] = ra
data["DEC"] = dec
...
if args.fullsurvey:
    ...
    outdir = args.outroot + "/test" + "/ramin" + str(ram) + "decmin" + str(decm) + wfistr + "/"
...
if args.palist is not None:  # this will only work for medium targets
    ...
    logger.info("palist provided, will set PA of repeated RA,DEC values to " + str(palist))
...
if args.fullsurvey is False:
    ...
    tls = tls[selreg]
logger.info("number of grism tiles to simulate after ra/dec cuts is " + str(len(tls)))
...
for chunk in range(0, Nchunk):
    ...
    rans, indices, cnts = np.unique(rand_indx, return_counts=True, return_inverse=True)
